...
int main() {
    ...
    // Read sample pixels
    MEASURE_KERNEL_TIME((read_sample_pixels<<<BLOCKS_PER_GRID_X, THREADS_PER_BLOCK_X>>>(d_image, w, h, total_npj, d_coordinates_flat, d_sample_pixels)), total_kernel_time);
    ...
    MEASURE_KERNEL_TIME((compute_sums<<<BLOCKS_PER_GRID_X, THREADS_PER_BLOCK_X>>>(total_npj, d_class_ids, d_sample_pixels, d_sums_r, d_sums_g, d_sums_b)), total_kernel_time);
    ...
    MEASURE_KERNEL_TIME((compute_averages<<<BLOCKS_PER_GRID_X, THREADS_PER_BLOCK_X>>>(nc, d_sums_r, d_sums_g, d_sums_b, d_npjs, d_avg_r, d_avg_g, d_avg_b)), total_kernel_time);
    ...
    MEASURE_KERNEL_TIME((compute_covariances<<<BLOCKS_PER_GRID_X, THREADS_PER_BLOCK_X>>>(total_npj, d_class_ids, d_sample_pixels, d_avg_r, d_avg_g, d_avg_b, d_covariance_matrices)), total_kernel_time);

    // Finalize covariance matrices
    MEASURE_KERNEL_TIME((finalize_covariances<<<BLOCKS_PER_GRID_X, THREADS_PER_BLOCK_X>>>(nc, d_covariance_matrices, d_npjs)), total_kernel_time);

    // Invert covariance matrices
    double *d_inverse_covariance_matrices;
    CSC(cudaMalloc(&d_inverse_covariance_matrices, nc * 9 * sizeof(double)));
    MEASURE_KERNEL_TIME((invert_covariances<<<BLOCKS_PER_GRID_X, THREADS_PER_BLOCK_X>>>(nc, d_covariance_matrices, d_inverse_covariance_matrices)), total_kernel_time);
