...
async def run_kernel(
    ...
    if all(item.get("test_result") for item in tasks):
        ...
        df_scores.to_csv(
            os.path.join(dir2save, f"stat_conditions_{bin_name}.csv"), index=False
        )
        print("SUCCESS")
    else:
        ...
        dir2save_failed = os.path.join(dir2save, f"failed_{bin_name}")
        os.makedirs(dir2save_failed, exist_ok=True)
        shutil.rmtree(dir2save_failed)
        os.makedirs(dir2save_failed, exist_ok=True)
        for fail in failed:
            with open(
                os.path.join(
                    dir2save_failed, f'{fail["idx"]}_first_vector_{bin_name}.npy'
                ),
                "wb",
            ) as f:
            ...
            with open(
                os.path.join(
                    dir2save_failed, f'{fail["idx"]}_second_vector_{bin_name}.npy'
                ),
                "wb",
            ) as f:
            ...
            with open(
                os.path.join(
                    dir2save_failed, f'{fail["idx"]}_result_vector_{bin_name}.npy'
                ),
                "wb",
            ) as f:
        ...
        df_failed.to_csv(os.path.join(dir2save, f"failed_{bin_name}.csv"), index=False)
    return df_scores
